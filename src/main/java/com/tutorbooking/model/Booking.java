...
public class Booking extends Session {
    ...
    public enum SessionType {
        ...
        REGULAR(60) {
            @Override
            public double calculateCost() {
                return 50.0; // Flat rate for regular sessions
            }
        },
        PREMIUM(90) {
            @Override
            public double calculateCost() {
                return 100.0; // Higher rate for premium sessions
            }
        };

        private final int baseMinutes;
        SessionType(int minutes) { this.baseMinutes = minutes; }
        public int getBaseDuration() { return baseMinutes; }
